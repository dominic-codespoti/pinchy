...
// ============================================================================
// Convenience Hook with Store Integration
// ============================================================================

import { useAgentStore, selectAgentListState } from '../store';

export interface UseFilteredAgentsFromStoreOptions {
  agents: Agent[];
}

export function useFilteredAgentsFromStore({
  agents,
}: UseFilteredAgentsFromStoreOptions): UseFilteredAgentsResult {
  const { searchQuery, filters, sort } = useAgentStore(selectAgentListState);

  return useFilteredAgents({
    agents,
    searchQuery,
    filters,
    sort,
...
export interface GroupedAgents {
  [groupId: string]: Agent[];
}

export interface UseGroupedAgentsResult {
...
export function useGroupedAgents(
  agents: Agent[],
  groupMapping: Map<string, string[]> // agentId -> groupIds
): UseGroupedAgentsResult {
  return useMemo(() => {
    ...
    // Initialize groups
    for (const [agentId, groupIds] of groupMapping.entries()) {
    ...
    for (const agent of agents) {
      const groupIds = groupMapping.get(agent.id);
    ...
    return {
      grouped,
      ungrouped,
      allGroups: Array.from(allGroups),
    };
  }, [agents, groupMapping]);
}
